...
def plot_box_overall(df_diags_all, out_plot_dir):
    ...
    fig, ax = plt.subplots(figsize=(max(7, 1.2*len(vi_order)), 6))
...
def main():
    ...
    rawvi_diags = []   # 新增：收集所有站点的诊断结果
    st_diags = []   # 新增：收集所有站点的诊断结果
    ...
    for (site,lc),info in sorted(files.items()):
        ...
        if args.st_vi or args.vpp_vi:
            # Identify VI columns (everything except 't')
            vi_columns = [c for c in merged.columns if c != "t"]

            dfst = []

            for vi_col in vi_columns:
                # Keep original column name
                merged_slice = merged[["t", vi_col]].copy()

                print(f"[INFO] site={site}, lc={lc}, vi={vi_col}")

                yfit_data, vpp_data, settings_df = Step_2_ts_function._ts_run_(merged_slice,args.vpp_vi)
                if args.st_vi and vi_col == 'GPP':
                    yfit_data = merged_slice

                yfit_data = yfit_data.copy()
                yfit_data["t"] = pd.to_datetime(yfit_data["t"])
                
                # NDVI + NIRv + EVI2
                dfst.append(yfit_data)

            merged_st = dfst[0]

            for df in dfst[1:]:
                merged_st = pd.merge(merged_st, df, on="t", how="outer")

            # Sort by time
            merged_st = merged_st.sort_values("t")

            # ---------- ST diagnostics (diag_st) ----------
            # We use the same diagnose_vi() helper as for raw data.
            # Requirement: use 'GPP' as reference and other VIs as predictors.

            if "GPP" in merged_st.columns:
                st_diags_list = []
                # All VI-like columns except time; we will skip 'GPP' inside
                vi_cols_st = [c for c in merged_st.columns if c not in ["t"]]

                for vi in vi_cols_st:
                    if vi == "GPP":
                        continue
                ...
                # Save per-site+LC diagnostics for fitted series
                out_csv_diag_st = Path(args.out_vi_dir) / f"Fitted_VIs_diag_{site}_{lc}.csv"
                diag_st.to_csv(out_csv_diag_st, index=False)
                print(f"[INFO] wrote {out_csv_diag_st}")

                # Collect for cross-site summary
                st_diags.append(diag_st)
            else:
                print(f"[WARN] GPP not in merged_st for {site},{lc}; skip ST diagnostics.")


                # Here you can process merged_slice
                # e.g. run_timesat(merged_slice, vi_name=vi_col)
        ...
        if args.raw_vi: # if want to save png
        ...
        #     plot_timeseries_for_site(site, lc, merged, args.out_plot_dir)
            # save merged CSV
            out_csv = Path(args.out_vi_dir) / f"Fitted_VIs_data_{site}_{lc}.csv"
    ...
    if rawvi_diags:
        df_diags_all = pd.concat(rawvi_diags, ignore_index=True)
        ...
        # Save the long-format diagnostics (one row per VI/site/LC)
        out_csv_long = Path(args.out_vi_dir) / "ST_VIs_diag_ALL_sites_long.csv"
        df_diags_st_all.to_csv(out_csv_long, index=False)
        print(f"[INFO] wrote {out_csv_long}")

        # ---------- build wide-format table with rho_gs only ----------
        # 1) for each (site, lc, VI) pick a single row (in case of duplicates)
        df_unique = (
            df_diags_st_all
            .sort_values("n", ascending=False)       # if duplicates, keep the largest n
            .drop_duplicates(subset=["site", "lc", "VI"])
        )

        # 2) pivot: rows = (site, lc), columns = VI, values = rho_gs
        ...
        # 3) compute a single n per (site, lc).
        # Here we take the *minimum* n across VIs as a conservative common sample size.
        n_per_site_lc = (
            df_unique
            .groupby(["site", "lc"])["n"]
            .min()
        ...
        # 4) attach n as the first data column
        wide = wide.join(n_per_site_lc)

        # move 'n' to be the first column after index
        cols = ["n"] + [c for c in wide.columns if c != "n"]
        wide = wide[cols]

        # 5) reset index to have columns: site, lc, n, VI columns ...
        wide = wide.reset_index()
        wide.columns.name = None  # drop the 'VI' name on the columns index

        out_csv_wide = Path(args.out_vi_dir) / "ST_VIs_diag_ALL_sites.csv"
        wide.to_csv(out_csv_wide, index=False)
        print(f"[INFO] wrote {out_csv_wide}")

        # Build 0/1 mask for best and statistically tied VIs
        out_csv_mask = Path(args.out_vi_dir) / "ST_VIs_diag_ALL_sites_bestmask.csv"
